				...
				<div class="row">
					...
					<div class="col-md-5">
						...
						<div class="form-group">
							...
							<label class="col-md-1">:</label>
							<div class="col-md-7">
								<input type="number" class="form-control" required v-model="transaction.In_Amount" style="display:none;" v-if="transaction.Tr_Type == 'In Cash'" v-bind:style="{display: transaction.Tr_Type == 'In Cash' ? '' : 'none'}">
								<input type="number" class="form-control" required v-model="transaction.Out_Amount" v-if="transaction.Tr_Type == 'Out Cash' || transaction.Tr_Type == ''" v-bind:style="{display: transaction.Tr_Type == 'Out Cash' || transaction.Tr_Type == '' ? '' : 'none'}">
							</div>
						</div>
						<div class="form-group">
						...
						<tr>
							...
							<td>
								<?php if ($this->session->userdata('accountType') != 'u') { ?>
									<a href="" title="Invoice" v-bind:href="`/cashTransactionInvoice/${row.Tr_SlNo}`" target="_blank"><i class="fa fa-file"></i></a>
									<a href="" title="Edit" @click.prevent="editTransaction(row)"><i class="fa fa-edit"></i></a>
									<a href="" title="Delete" @click.prevent="deleteTransaction(row.Tr_SlNo)"><i class="fa fa-trash"></i></a>
								<?php } ?>
							</td>
						</tr>
					</template>
