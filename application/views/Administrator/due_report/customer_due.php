		...
		<div class="form-group">
			...
			<div class="col-sm-2">
				<select class="form-control" v-model="searchType" v-on:change="onChangeSearchType" style="padding:0px;">
					...
					<option value="shop_sale">By Shop Sale</option>
				</select>
			</div>
		</div>
		<div class="form-group" style="display: none" v-bind:style="{display: searchType != 'all' ? '' : 'none'}">
			<label class="col-sm-2 control-label no-padding-right" style="width: 100px;">Select Renter</label>
			<div class="col-sm-3">
		...
		methods: {
			onChangeSearchType() {
				if (this.searchType == 'customer') {
					...
					this.getAreas();
				} else if (this.searchType == 'shop_sale') {
					this.getShopSaleCustomers();
				}
			...
			getShopSaleCustomers() {
				axios.get('/get_shop_sale_customers').then(res => {
					this.customers = res.data;
				})
			},
			getDues() {
				...
				let renterId = this.selectedCustomer == null ? null : this.selectedCustomer.Renter_Id;


				if (this.searchType == 'shop_sale') {
