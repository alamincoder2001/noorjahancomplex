...
class Model_Table extends CI_Model
{
    ...
    {
        $invoice = 'R-' . date('Y') . "00001";
        $year = date('Y');
        $rents = $this->db->query("select * from tbl_rent_generate rg where rg.invoice_no like 'R-$year%'");
        if ($rents->num_rows() != 0) {
            $newRentId = $rents->num_rows() + 1;
            $zeros = array('0', '00', '000', '0000', '00000');
            $invoice = 'R-' . date('Y') . (strlen($newRentId) > count($zeros) ? $newRentId : $zeros[count($zeros) - strlen($newRentId)] . $newRentId);
        }
        return $invoice;
    }

    public function generateShopSaleInvoice()
    {
        $invoice = 'S-' . date('Y') . "0001";
        $year = date('Y');
        $sale = $this->db->query("select * from tbl_shop_sales ORDER BY Shop_Sale_id DESC LIMIT 1 ")->row();
        if ($sale) {
            $newInvoice = substr($sale->Invoice, 2) + 1;
            $invoice = 'S-' . $newInvoice;
        }
        return $invoice;
    }
    ...
    public function generateCustomerCode()
    {
        ...
        if ($lastCustomer->num_rows() != 0) {
            ...
            $zeros = array('0', '00', '000', '0000');
            $customerCode = 'R' . (strlen($newCustomerId) > count($zeros) ? $newCustomerId : $zeros[count($zeros) - strlen($newCustomerId)] . $newCustomerId);
        }
    ...
    public function renterDue($clauses = "", $dateFrom = "", $dateTo = "")
    {
        $branchId = $this->session->userdata('BRANCHid');
        ...
        $shope_sale_date = '';
        if ($dateFrom != '' && $dateTo != '') {
            $rent_gen_date .= " and rg.generate_date between '$dateFrom' and '$dateTo' ";
            $cus_pay_date .= " and cp.CPayment_date between '$dateFrom' and '$dateTo' ";
            $sm_date .= " and smr.SaleMaster_SaleDate between '$dateFrom' and '$dateTo' ";
            $shope_sale_date .= " and ss.Date between '$dateFrom' and '$dateTo' ";
        }
        ...
        $dueResult = $this->db->query("
            select
                ...
                (select ifnull(sum(rg.total_amount), 0.00) + ifnull(c.previous_due, 0.00)
                    ...
                    where rg.renter_id = c.Customer_SlNo
                    and rg.Status = 'a'
                    $rent_gen_date ) as billAmount,

                (select ifnull(sum(rg.paid_amount), 0.00)
                    from tbl_rent_generate rg
                    where rg.renter_id = c.Customer_SlNo
                    and rg.Status = 'a'
                    $rent_gen_date) as invoicePaid,

                (select ifnull(sum(cp.CPayment_amount), 0.00) 
                    from tbl_customer_payment cp 
                    where cp.CPayment_customerID = c.Customer_SlNo and cp.CPayment_TransactionType = 'CR'
                    and cp.CPayment_status = 'a'
                    $cus_pay_date) as cashReceived,
                ...
                (select ifnull(sum(cp.CPayment_amount), 0.00) 
                    ...
                    and cp.CPayment_TransactionType = 'CP'
                    and cp.CPayment_status = 'a'
                    $cus_pay_date) as paidOutAmount,

                (select ifnull(sum(sr.SaleReturn_ReturnAmount), 0.00) 
                    from tbl_salereturn sr 
                    join tbl_salesmaster smr on smr.SaleMaster_InvoiceNo = sr.SaleMaster_InvoiceNo 
                    where smr.SalseCustomer_IDNo = c.Customer_SlNo
                    $sm_date) as returnedAmount,
                
                (select ifnull(sum(ss.Due_Amount), 0.00) 
                    from tbl_shop_sales ss 
                    where ss.Renter_Id = c.Customer_SlNo
                    $shope_sale_date) as shopSaleAmount,

                (select invoicePaid + cashReceived) as paidAmount,

                (select (billAmount + paidOutAmount + shopSaleAmount) - (paidAmount + returnedAmount)) as dueAmount,
                ...
                (
                    select ifnull(sum(cp.CPayment_adjustment_amount), 0.00)
                    from tbl_customer_payment cp
                    where cp.CPayment_customerID = c.Customer_SlNo
                    and cp.CPayment_status = 'a'
                ) as payAdjustment,
                ...
                (
                    select AdvanceAmount - payAdjustment
                ) as RestOfAdjustment
