...
class Customer extends CI_Controller
{
    ...
    public function getCustomers()
    {
        ...
        if (isset($data->searchType) && $data->searchType == 'birthday') {
            if (isset($data->dateFrom) && $data->dateFrom != '' && isset($data->dateTo) && $data->dateTo != '') {
                $customerTypeClause .= " and DATE_FORMAT(c.renter_birthday, '%m-%d') = DATE_FORMAT('$data->dateFrom', '%m-%d')";
            }
        }
        if (isset($data->searchType) && $data->searchType == 'marriage_day') {
            if (isset($data->dateFrom) && $data->dateFrom != '' && isset($data->dateTo) && $data->dateTo != '') {
                $customerTypeClause .= " and DATE_FORMAT(c.renter_marriage_day, '%m-%d') = DATE_FORMAT('$data->dateFrom', '%m-%d')";
            }
        }
        ...
        foreach ($customers as $value) {
            $customerId = $value->Customer_SlNo;
            $value->advance = $this->db->query("select * from tbl_advance_payments where renter_id = ?", $customerId)->result();
        }
    ...
    public function getCustomerDue()
    {
        ...
        // $dateClause = '';
        if (isset($data->dateFrom) && $data->dateFrom != '' && isset($data->dateTo) && $data->dateTo != '') {
            $dueResult = $this->mt->renterDue($clauses, $data->dateFrom, $data->dateTo);
        } else {
    ...
    public function getCostCenterColumnWise()
    {
        $data = json_decode($this->input->raw_input_stream);
        ...
        // $dateClause = '';
        // if (isset($data->dateFrom) && $data->dateFrom != '' && isset($data->dateTo) && $data->dateTo != '') {
        //     $dateClause = " and cp.CPayment_date between '$data->dateFrom' and '$data->dateTo'";
        // }

        $result = $this->db->query("SELECT c.*,

            (select ifnull(sum(rg.ac_bill),0)
            FROM tbl_rent_generate rg
            WHERE rg.renter_id = c.Customer_SlNo
            and rg.status = 'a'
            and rg.generate_date BETWEEN '$data->dateFrom' and '$data->dateTo') as ac_bill,
            
            (select ifnull(sum(rg.shop_electricity_bill),0)
            FROM tbl_rent_generate rg
            WHERE rg.renter_id = c.Customer_SlNo
            and rg.status = 'a'
            and rg.generate_date BETWEEN '$data->dateFrom' and '$data->dateTo') as electricity_bill,
            
            (select ifnull(sum(rg.service_charge),0)
            FROM tbl_rent_generate rg
            WHERE rg.renter_id = c.Customer_SlNo
            and rg.status = 'a'
            and rg.generate_date BETWEEN '$data->dateFrom' and '$data->dateTo') as service_charge,
            
            (select ifnull(sum(rg.shop_rent),0)
            FROM tbl_rent_generate rg
            WHERE rg.renter_id = c.Customer_SlNo
            and rg.status = 'a'
            and rg.generate_date BETWEEN '$data->dateFrom' and '$data->dateTo') as shop_rent,
            
            (select ifnull(sum(ss.Sale_Amount),0)
            FROM tbl_shop_sales ss
            WHERE ss.Renter_Id = c.Customer_SlNo
            and ss.Status = 'a'
            and ss.Date BETWEEN '$data->dateFrom' and '$data->dateTo') as shop_sale_amt,
            
            (select ac_bill + electricity_bill + service_charge + shop_rent + shop_sale_amt) as total_bill_generate
        ...
        $result = array_map(function ($value) use ($dateFrom, $dateTo) {
            $clauses2 = " and c.Customer_SlNo = '$value->Customer_SlNo'";

            $due_amt = $this->mt->renterDue($clauses2, $dateFrom, $dateTo);
            $value->due_amount = $due_amt[0]->dueAmount;
            $value->Interest = $value->due_amount * .15;
            $value->total = $value->due_amount + $value->Interest;
            return $value;
        }, $result);
    ...
    public function getCustomerDueShopSale()
    {
        $data = json_decode($this->input->raw_input_stream);

        $clauses = "";
        if (isset($data->renterId) && $data->renterId != null) {
            $clauses .= " and Renter_Id = '$data->renterId'";
        }

        $dueResult = $this->db->query("select * from (    
            select
                ...
                (
                    ...
                    where cp.Shop_Sale_id = ss.Shop_Sale_id
                ) as payment_amount,
                (select Due_Amount - payment_amount) as dueAmount
            from tbl_shop_sales ss
            left join tbl_customer c on c.Customer_SlNo = ss.Renter_Id
            where ss.Status = 'a'
            and ss.Brunch_Id = 1
             ) as tbl
    ...
    public function addCustomerPayment()
    {
        ...
        try {
            ...
            $query = $this->db->query("select rent_id from tbl_customer_payment where rent_id = '$paymentObj->rent_id'");

            $payment = (array)$paymentObj;
            if ($query->num_rows() > 0) {
                $payment['CPayment_adjustment_amount'] = 0.00;
            }
            ...
            if ($paymentObj->CPayment_TransactionType == 'CR') {
                ...
                $message = "মি: {$sendToName},\nআপনার কর্তৃক প্রেমেন্ট রিসিভ: tk.{$paymentObj->CPayment_amount}.\nআপনার অবশিষ্ট বাকীর পরিমান: tk.{$currentDue}";
    ...
    public function addCustomer()
    {
        ...
        try {
            ...
            if ($this->input->raw_input_stream) {
                ...
                $customerObj = $customerObj->data ?? null;
            } else {
                $customerObj = json_decode($this->input->post('data'));
            }

            $customerObj->Customer_Code = $customerObj->Customer_Code ?? '';
            ...
            if ($duplicateMobileQuery->num_rows() > 0) {
                ...
                $customer["Customer_Credit_Limit"] = 1000000000;
                ...
                foreach ($carts as $value) {
                    ...
                    $this->db->insert('tbl_advance_payments', $advancePayment);
                }
            }
            ...
            $res = ['success' => true, 'message' => 'Renter added successfully', 'customerCode' => $this->mt->generateCustomerCode()];
        } catch (Exception $ex) {
            $res = ['success' => false, 'message' => $ex->getMessage()];
        }
    ...
    public function updateCustomer()
    {
        ...
        try {
            ...
            $this->db->where('renter_id', $customerId)->delete('tbl_advance_payments');
            $carts = json_decode($this->input->post('cart'));
            ...
            $res = ['success' => true, 'message' => 'Renter updated successfully', 'customerCode' => $this->mt->generateCustomerCode()];
        } catch (Exception $ex) {
            $res = ['success' => false, 'message' => $ex->getMessage()];
        }
    ...
    function Cost_Center_Report()
    {
        ...
        $data['title'] = "Cost Center Report";
        $branch_id = $this->session->userdata('BRANCHid');

        $data['content'] = $this->load->view('Administrator/payment_reports/costCenterReport', $data, TRUE);
        $this->load->view('Administrator/index', $data);
    }
    function CostCenterColumnWise()
    {
        $access = $this->mt->userAccess();
        if (!$access) {
            redirect(base_url());
        }
        $data['title'] = "Cost Center Column Wise";
        $branch_id = $this->session->userdata('BRANCHid');

        $data['content'] = $this->load->view('Administrator/payment_reports/costCenterColumnWise', $data, TRUE);
        $this->load->view('Administrator/index', $data);
    }
    ...
    function getCustomerLedger()
    {
        ...
        $payments = $this->db->query("
            select 
                ...
                concat('Rents ', rg.invoice_no) as description,
                rg.total_amount as bill,
                rg.paid_amount as paid,
                rg.due_amount as due,
                0.00 as returned,
                0.00 as paid_out,
                0.00 as balance
            from tbl_rent_generate rg
            ...
            select
                ...
                ss.Date as date,
                concat('Paid - ', 
                    case ss.Payment_Type
                        when 'bank' then concat('Bank - ', ba.account_name, ' - ', ba.account_number, ' - ', ba.bank_name)
                        else 'Cash'
                    end, ' ', ss.Notes
                ) as description,
                ss.Sale_Amount as bill,
                ss.Paid_Amount as paid,
                ss.Due_Amount as due,
                0.00 as returned,
                0.00 as paid_out,
                0.00 as balance
            from tbl_shop_sales ss
            left join tbl_bank_accounts ba on ba.account_id = ss.Bank_Account_Id
            where ss.Transaction_Type = 'CR'
            and ss.Renter_Id = '$data->customerId'
            and ss.Status = 'a'
    ...
    public function customerPaymentHistory()
    {
        ...
        $data['title'] = "Renter Payment History";
    ...
    public function shopSalePage()
    {
        $access = $this->mt->userAccess();
        if (!$access) {
            redirect(base_url());
        }
        $data['title'] = "Shop Sale";
        $data['content'] = $this->load->view('Administrator/due_report/shopSalePage', $data, TRUE);
        $this->load->view('Administrator/index', $data);
    }

    public function addShopSale()
    {
        $res = ['success' => false, 'message' => ''];
        $saleObj = json_decode($this->input->raw_input_stream);

        try {

            $sale = (array)$saleObj;
            unset($sale['Shop_Sale_id']);
            $sale['Invoice'] = $this->mt->generateShopSaleInvoice();
            $sale['Status'] = 'a';
            $sale['Addby'] = $this->session->userdata("FullName");
            $sale['Add_Date'] = date('Y-m-d H:i:s');
            $sale['Brunch_Id'] = $this->session->userdata("BRANCHid");

            $this->db->insert('tbl_shop_sales', $sale);
            ...
            $res = ['success' => true, 'message' => 'Shop sale added successfully', 'saleId' => $saleId];
        } catch (Exception $ex) {
            $res = ['success' => false, 'message' => $ex->getMessage()];
        }
    ...
    public function updateShopSale()
    {
        $res = ['success' => false, 'message' => ''];
        $saleObj = json_decode($this->input->raw_input_stream);
        $saleId = $saleObj->Shop_Sale_id;
        try {

            $sale = (array)$saleObj;
            unset($sale['Shop_Sale_id']);
            $sale['Update_By'] = $this->session->userdata("FullName");
            $sale['Update_Date'] = date('Y-m-d H:i:s');

            $this->db->where('Shop_Sale_id', $saleId)->update('tbl_shop_sales', $sale);

            $res = ['success' => true, 'message' => 'Shop sale updated successfully', 'saleId' => $saleId];
        } catch (Exception $ex) {
            $res = ['success' => false, 'message' => $ex->getMessage()];
        }
    ...
    public function deleteSalePage()
    {
        $res = ['success' => false, 'message' => ''];
        try {
            $data = json_decode($this->input->raw_input_stream);

            $this->db->set(['Status' => 'd'])->where('Shop_Sale_id', $data->saleId)->update('tbl_shop_sales');

            $res = ['success' => true, 'message' => 'Shop sale deleted successfully'];
        } catch (Exception $ex) {
            $res = ['success' => false, 'message' => $ex->getMessage()];
        }
    ...
    public function getSalePage()
    {
        ...
        $shopSales = $this->db->query("SELECT ss.*,
            c.Customer_Name,
            c.Customer_Code,
            s.shop_name,
            s.shop_no,
            ...
            FROM tbl_shop_sales ss
            LEFT JOIN tbl_customer c on c.Customer_SlNo = ss.Renter_Id
            LEFT JOIN tbl_shops s on s.id = ss.Office_or_Shop_No
            LEFT JOIN tbl_bank_accounts ba on ba.account_id = ss.Bank_Account_Id 
            WHERE ss.Status = 'a'
            order by ss.Shop_Sale_id desc")->result();

        echo json_encode($shopSales);
    }

    public function getShopSaleCustomers()
    {
        $data = json_decode($this->input->raw_input_stream);
